...
# def addnoise(image,percentage):
#     img_noiseing=image.copy()
#     h, w = image.shape[0], image.shape[1]
#     num=int(percentage*image.shape[0]*image.shape[1])
#     for i in range(num):
#         x=np.random.randint(0,h)
#         y=np.random.randint(0,w)
#         img_noiseing[x][y][np.random.randint(3)]=np.random.randn(1)[0]
#         return img_noiseing
...
def get_model():
    ...
    weights_path = (r"models/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5")
...
def generate_arrays_from_file(lines, batch_size):
    ...
    while True:
        ...
        for t in range(batch_size):
            ...
            # 根据图片名字读取图片
            img = Image.open('Datasets/dataset/trainData/train' + '/' + train_x_name)
            # img.show()
            # print(img.size)
            ...
            # data augmentation
            #img_array1 = addnoise(image=img_array)
            img_array1 = flip(image=img_array)
            img_array2 = shift(image=img_array)
            #img_array3 = crop(image=img_array)
            ...
            img_array1 = img_array1 / 255
            img_array2 = img_array2 / 255 # 标准化 img_array.shape=（416，416，3）
            ...
            train_data.append(img_array1)
            train_data.append(img_array2)# 添加到训练样本

            # 2. 获取训练样本标签的名字,名字如：42.png；标签是一张图片
            train_y_name = lines[read_line].split(";")[1].replace('\n', '')

            # 根据图片名字读取图片
            img = Image.open('Datasets/dataset/trainData/label' + '/' + train_y_name)
            # img.show()
            # print(train_y_name)
            img = img.resize((int(WIDHT / 2), int(HEIGHT / 2)))  # 改变图片大小 -> (208,208)。标签（图片）大小208*208
            img_array = np.array(img)
            #img_array1 = addnoise(image=img_array)
            img_array1 = flip(image=img_array)  # data augmentation
            img_array2 = shift(image=img_array)
            #img_array2 = crop(image=img_array)
            # 图片转换成数组 img_array.shape=(208,208,3)
            ...
            labels2 = np.zeros((int(HEIGHT / 2), int(WIDHT / 2), CLASS_NUMBERS))
            #labels3 = np.zeros((int(HEIGHT / 2), int(WIDHT / 2), CLASS_NUMBERS))
            ...
            for cn in range(CLASS_NUMBERS):  # range(0,2) => 0,1
                ...
                labels1[:, :, cn] = (img_array1[:] == cn).astype(int)
                labels2[:, :, cn] = (img_array2[:] == cn).astype(int)
                #labels3[:, :, cn] = (img_array2[:] == cn).astype(int)
            labels = np.reshape(labels, (-1, CLASS_NUMBERS))  # (208,208,2)=>(208*208,2)

            train_label.append(labels)
            train_label.append(labels1)
            train_label.append(labels2)
            #train_label.append(labels3)
...
def main():
    ...
    # 生成样本和标签
    generate_arrays_from_file(lines, batch_size=3)

    # 训练
    model.fit_generator(generate_arrays_from_file(lines[:train_nums], batch_size),
                        steps_per_epoch=max(1, train_nums*3 // batch_size),
