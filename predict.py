...
def get_model():
    ...
    model.load_weights('callbacks/mobile2021322.h5')
    return model


def precess_img(img):
    """
    reshape the image and normalization
...
def predicting(model):
    ...
    test_data = os.listdir(test_data_path)  # image
    print(test_data)
    ...
    for test_name in test_data:
        ...
        test_img_array = np.array(test_img)  # iamge to array
        # print(test_img_array.shape)
        ...
        save_path = os.path.join(save_path, 'predict_mobile')
        ...
        save_path = 'Datasets/dataset/testData'
        save_path = os.path.join(save_path, 'imgout_mobile')
        if not os.path.exists(save_path):
            os.mkdir(save_path)
